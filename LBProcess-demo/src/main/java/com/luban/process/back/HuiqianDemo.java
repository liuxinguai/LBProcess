...
public class HuiqianDemo {
    ...
    public static void main(String[] args) {
        ...
        variables.put("assignee2List", Arrays.asList("李四","王五","五七"));
...
//            engine.getTaskService().complete(task.getId());
        ...
        tasks = engine.getTaskService().createTaskQuery().processInstanceId(instance.getProcessInstanceId()).taskAssignee("五七").list();
        tasks.forEach(task -> {
            log.info("五七处理任务:{}",task.getName());
            engine.getTaskService().complete(task.getId());
        });

        System.out.println("重新会签------------------------------------");
        tasks = engine.getTaskService().createTaskQuery().processInstanceId(instance.getProcessInstanceId()).taskAssignee("张三").list();
        tasks.forEach(task -> {
            log.info("张三处理任务:{}",task.getName());
            engine.getTaskService().complete(task.getId());
        });
...
//            back(task,lastTaskId.get(),engine);
    ...
    public static void back(Task task, String lastTaskId, ProcessEngine engine) {
        ...
        HistoricActivityInstance lastInstance = historicActivityInstances.stream()
            ...
            .findAny().orElseThrow(() -> new RuntimeException("要回退的任务不存在!"));
