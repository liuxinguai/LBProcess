...
public class Demo3 {
    ...
    public static void main(String[] args) {
        String processKey = "gateway3", resource = "gateway/gateway3.bpmn20.xml", tenantId = "002", processName = "gateway3子流程",processInstanceId = "275001";
        ...
        variables.put("subAssignee3","王五1");
        variables.put("subAssignee4","赵六1");
...
//        });
        tasks = engine.getTaskService().createTaskQuery()
                .processInstanceId(processInstance.getProcessInstanceId()).taskAssignee("王五1").list();
        tasks.forEach(task -> {
            log.info("王五1处理任务:{}",task);
            engine.getTaskService().complete(task.getId());
//            lastId.set(task.getId());
        });
        tasks = engine.getTaskService().createTaskQuery()
                .processInstanceId(processInstance.getProcessInstanceId()).taskAssignee("赵六1").list();
        tasks.forEach(task -> {
            log.info("赵六1处理任务:{}",task);
            engine.getTaskService().complete(task.getId());
    ...
    private static void back(Task task, String lastId, ProcessEngine engine) {
        ...
        if (flag == 3 || flag == 0) { //全为子流程
            ...
            if (!((lastNode instanceof Activity) && ((Activity) lastNode).hasMultiInstanceLoopCharacteristics())) {
                Task nextTask = engine.getTaskService().createTaskQuery().processInstanceId(task.getProcessInstanceId()).taskDefinitionKey(lastNode.getId()).singleResult();
